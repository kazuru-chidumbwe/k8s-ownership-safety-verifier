...
| Tag | Commit | Purpose |
| --- | --- | --- |
| [`v0.1.10`](https://github.com/kazuru-chidumbwe/k8s-ownership-safety-verifier/releases/tag/v0.1.10) | *(this release)* | **Current cite** — Kind pin in archive metadata (`cluster.json` / `kind_node_image`) |
...
Short hashes are what `git rev-parse --short <tag>^{commit}` reports for that tag. After cutting `v0.1.10`, replace *(this release)* with that short hash.
