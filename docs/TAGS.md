...
| Tag | Commit | Purpose |
| --- | --- | --- |
| `v0.1.3` | _(this release — run `git rev-parse --short HEAD` on the tag)_ | **SoftwareX / blog cite pin** (O2 `(resource, uid)` identity fix) |
...
- Sanity-check any cited SemVer: `git rev-parse --short <tag>^{commit}` must match this table on `main` (and `git cat-file -t <hash>` must succeed). Phantom pre-rewrite `1d51980` must not resolve.
