...
| Cross-resource UID | `fixtures/o2-cross-resource-shared-uid.jsonl` → `results/smoke-o2-cross-resource-uid.json` | **PASS** (no false O2) |
...
2. **O2 FAIL** — same `(resource, uid)` ControllerRef A→B with no orphan/DELETE → checker FAILs O2.  
3. **Intended PASS** — A→B after orphan → not O2; **PASS**.  
4. **Cross-resource UID PASS** — pod and ReplicaSet share a UID string with different controllers → **PASS** (O2 state is resource-scoped).  
5. **Kind PASS** — Deployment create → scale 2→3; polled ownership events; **O1 PASS; O2 PASS**.
